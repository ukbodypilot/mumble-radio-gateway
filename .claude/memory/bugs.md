...
**Fix:** `on_ack` now merges any command result dict (minus `ok`/`error`/`response`) into `_link_last_status`. Plus client-side reconciliation in `makePotKnob` — the knob holds the user's value until the poll echoes it back (±1), so a rejected command (e.g. CI-V offline) no longer snaps it.
**Files:** `gateway_setup.py` (`on_ack`), `web_pages/ic7100.html` (`makePotKnob` pending logic).
**Pattern / how to avoid:** When a UI mirrors backend state by polling, every backend mutation must update the polled cache immediately — periodic snapshots aren't enough. Optimistic UI also needs confirmation-based reconciliation, not a fixed-time hold-off.

## IC-7100 TX-interlock checkboxes appeared mutually exclusive — poll clobbered un-focused controls (2026-05-20)
**Symptom:** On `/ic7100`, ticking one TX-interlock checkbox (HF / VHF-UHF) unticked the other — they behaved like radio buttons, either direction. The status-bar interlock dots never went green either.
**Root cause:** Both symptoms, one cause — the updated `ic7100_link_plugin.py` had not been deployed to the MX endpoint, so endpoint status carried no `tx_allow_hf`/`tx_allow_vu` keys. `renderStatus` synced the checkboxes every 1 Hz with `checkbox.checked = !!s.tx_allow_X`; with the value `undefined` that forced every checkbox **not currently focused** to `false`. Tick box A (focused — survives the poll), then tick box B → the next poll clears A. It looked like mutual exclusivity but was the poll clobbering a controlled input whose backing key was absent. Dots: `setDot(undefined ? 'ok' : 'err')` → permanently red.
**Fix:** `syncChk` now writes the checkbox only when `typeof val === 'boolean'` — an absent key means "leave it alone", not "false". `setDot` gained a third grey/'pending' state for `undefined`. And the actual unblock: deploy the plugin so the keys arrive.
**Files:** `web_pages/ic7100.html` (`renderStatus` syncChk/setDot).
**Pattern / how to avoid:** A poll-driven sync that overwrites a user-editable control with `!!value` will fight the user whenever the backend omits that key (old endpoint code, command not delivered, plugin not deployed). Always guard status→control sync with a real type check and treat "key absent" as no-op. Applies to any status-synced input, not just checkboxes.

## IC-7100 HF panel + FM-squelch web commands never wired in route dispatcher (2026-05-20)
**Symptom:** Every Phase C HF panel control on the `/ic7100` page — split, RIT, XIT, AGC, NB, NR, preamp, atten, IF-shift, filter — plus the FM squelch-type selector, DTCS code/polarity, and the squelch slider, did nothing. Each button POST to `/ic7100cmd` came back `{'ok': false, 'error': 'unknown cmd: <name>'}`. Freq/mode/PTT/CTCSS/gain worked fine.
**Root cause:** `handle_ic7100cmd()` in `web_routes_radio.py` is an explicit per-command dispatcher — an allowlist of `elif cmd == '...'` branches with an `else: unknown cmd` fallthrough. When Phase C shipped (2026-05-19) the plugin-side `IC7100Plugin.execute()` action handlers AND the GUI buttons were both built, but the gateway-side route dispatcher between them was never extended — only `ptt/freq/mode/ctcss/rx_gain/tx_gain/mute/cat/status` had branches. Classic cross-cutting gap: a feature that touched plugin + UI but skipped the middle layer. Not caught by tests because `test_ic7100_civ.py` exercises `execute()` directly and never goes through the HTTP route.
**Fix:** Added `_IC7100_HF_CMDS` frozenset (the 13 commands) and a single generic branch in `handle_ic7100cmd` that forwards the GUI's JSON payload verbatim to the endpoint — the GUI already sends correctly-keyed payloads and `execute()` validates. Fixed 2026-05-20, same commit as the TX antenna-port interlock work.
**Files:** `web_routes_radio.py` (`handle_ic7100cmd`, `_IC7100_HF_CMDS`).
**How to diagnose next time:** an IC-7100 panel command is a 3-legged chain — `IC7100Plugin.execute()` action + `handle_ic7100cmd` route branch + `web_pages/ic7100.html` `icCmd()` call. A command missing the route branch fails silently as `unknown cmd`, visible only in the browser feedback toast. `/ic7100status` does NOT have this gap — `handle_ic7100status` forwards the whole status dict, so new status keys flow through automatically.
**Related:** the "half-built plugin = no panel + cross-cutting not done" pattern (`feedback_half_built_plugin`).
