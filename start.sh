...
if [ "$CLAUDE_RUNNING" = false ]; then
    ...
    if [ -n "$CLAUDE_BIN" ]; then
        if command -v xfce4-terminal > /dev/null 2>&1; then
            xfce4-terminal --geometry=88x24 --working-directory="$GATEWAY_DIR" -e "$CLAUDE_BIN" &
            disown
        elif command -v lxterminal > /dev/null 2>&1; then
            lxterminal --geometry=88x24 --working-directory="$GATEWAY_DIR" -e "$CLAUDE_BIN" &
            disown
        elif command -v x-terminal-emulator > /dev/null 2>&1; then
            cd "$GATEWAY_DIR" && x-terminal-emulator --geometry=88x24 -e "$CLAUDE_BIN" &
            disown
            cd - > /dev/null
        elif command -v gnome-terminal > /dev/null 2>&1; then
            gnome-terminal --geometry=88x24 --working-directory="$GATEWAY_DIR" -- "$CLAUDE_BIN" &
            disown
        else
